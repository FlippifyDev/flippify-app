...
const DisabledSideBarButton: React.FC<DisabledSideBarButtonProps> = ({
  ...
  return (
    <div className="relative group">
      <button
        className={`relative z-50 w-full flex justify-start items-center gap-4 py-2 ${
          isSidebarOpen ? "px-2" : "pl-2"
      ...
      >
        ...
        <span
          className={`absolute left-14 text-base text-left ${
      ...
      {isSidebarOpen && tooltip && (
        <div className="absolute z-50 bottom-full mb-2 left-1/2 transform -translate-x-1/2 p-2 bg-gray-800 text-white text-sm rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
          {tooltip}
        </div>
      )}
    </div>
