...
const NabarItem: React.FC<NavbarItemProps> = ({ title, icon, isConnected, isSubscribed }) => {
    return (
        <div className="relative inline-block group">
            ...
            <div className="absolute left-1/4 transform -translate-x-1/2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                <div className="bg-gray-800 text-white text-xs rounded py-1 px-2 text-nowrap">
                    {!isSubscribed ? "Subscription Required" :
                        isConnected ? title : "No connected account"
                    }
                </div>
            </div>
        </div>
