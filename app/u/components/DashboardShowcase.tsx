import React from "react";
import { BentoGrid, BentoGridItem } from "../../../components/ui/bento-grid";
import { IconClipboardCopy, IconFileBroken, IconSignature, IconTableColumn } from "@tabler/icons-react";

const DashboardShowcase: React.FC = () => {
  return (
    <BentoGrid className="max-w-4xl mx-auto md:auto-rows-[20rem] gap-6 p-4">
      {items.map((item, i) => (
        <BentoGridItem
          key={i}
          title={item.title}
          description={item.description}
          header={item.header}
          className={item.className}
          icon={item.icon}
        />
      ))}
    </BentoGrid>
  );
};

const Skeleton = () => (
  <div className="flex flex-1 w-full h-full min-h-[6rem] rounded-xl dark:bg-dot-white/[0.2] bg-dot-black/[0.2] border border-transparent dark:border-white/[0.2] bg-neutral-100 dark:bg-black"></div>
);

const items = [
  {
    title: "Deal-Finding Bots",
    description: "Our bots help you find the best deals across various categories.",
    header: <Skeleton />,
    className: "md:col-span-2",
    icon: <IconClipboardCopy className="h-4 w-4 text-houseBlue" />,
  },
  {
    title: "Category Support",
    description: "Wide range of supported categories for reselling.",
    header: <Skeleton />,
    className: "md:col-span-1",
    icon: <IconFileBroken className="h-4 w-4 text-houseBlue" />,
  },
  {
    title: "eBay AI Automation",
    description: "In-progress automation for end-to-end eBay reselling.",
    header: <Skeleton />,
    className: "md:col-span-1",
    icon: <IconSignature className="h-4 w-4 text-houseBlue" />,
  },
  {
    title: "Hands-Free Reselling",
    description: "Let us handle everything from purchase to profit.",
    header: <Skeleton />,
    className: "md:col-span-2",
    icon: <IconTableColumn className="h-4 w-4 text-houseBlue" />,
  },
];

export default DashboardShowcase;
